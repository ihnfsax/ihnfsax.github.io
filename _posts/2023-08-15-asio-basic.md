...
- **Proactor**：调用异步事件解复用器以使事件出队，并分派完成句柄（即调用与事件相关联的函数对象）。这个抽象为 `io_context` 类。
- **完成句柄 (Completion Handler)**：处理异步操作的结果，是函数对象。
- **初始化器 (Initiator)**：初始化异步操作的代码。

Windows 上，由于 **IO Completion Port (IOCP)** 系统级异步 I/O 的存在，该 Proactor 模型得以实现。首先，异步操作执行单元属于 Windows 操作系统，即异步操作是由系统完成的。比如从套接字读数据，数据放入用户缓冲区以及之前的工作都是由操作系统完成的（异步 I/O 直接将数据拷贝到用户缓冲区，不经过内核，性能更好）。其次，完成事件队列也是由 Windows 系统管理，我们只需要用 `GetQueuedCompletionStatus` 即可获得一个完成事件，这就是 Asio 在异步事件解复用器中的做法。
...
- 完成事件队列：一个链表形式的完成句柄队列。
- 异步事件解复用器：通过条件变量指示完成事件队列中是否有完成句柄。

从这里的分析可以看出，Reactor 和 Proactor 的区别在于完整的 I/O 操作（包括在用户缓冲区准备好或消耗完数据）是否是由第三方完成的。如果都是第三方完成的，就是 Proactor，否则是 Reactor。从 Asio 使用者的角度来看，第三方是 Asio 库和操作系统，所以无论是在什么平台上，Asio 都是 Proactor 模式。从 Asio 的实现角度来看，第三方只有操作系统，这时 Asio 在 Windows 下是 Proactor，在 Linux/Unix 下是 Reactor。

### Linux 下多线程 `io_context.run()`

在 Linux 下，`epoll_wait` 仅发生在执行 `io_context.run()` 后。如果有多个线程对同一个 `io_context` 调用了 `run()` 方法，那么同一时间也只有一个线程在 `epoll_wait`。事件触发导致 `epoll_wait` 返回后，对套接字的读写操作可以由另一个线程进行。当操作完成后，完成句柄被放入完成事件队列，因此完成句柄的执行线程又可以前面两个线程都不相同。总的来说，多线程情况下，下面三类工作可以由不同线程完成：

1. 调用 `epoll_wait`，等待事件出现。
2. 处理事件，调用 `accept`, `recvfrom`, `sendto` 等函数。
3. 从完成事件队列取出完成句柄，执行完成句柄。
